...
*/
    ...
    public static void startLoto() {
        ...
        int ticketArr[] = new int[slots];// hem loto icin hem de kullanici icin ayni buyuklukte arraylar olusturulur kullanicinin sonucu
        int lotoArr[] = new int[slots];// buyuklugu bildirmem gerek, bunu da kullanici vermisti slots adinda. loto sonucu var.

        Random rnd = new Random();
        int rnd1 = 0;
        int rnd2 = 0;//default deger atadik her ikisine de

        //tekrarli sayi olmasin istiyorum tekrarli mi diye bakabilecegim konteynir boolean olmali
        boolean isRepeat;//ayni sayisi bir daha uretmesin

        for (int i = 0; i < slots; i++) {
            do {
                isRepeat = false;//tekrarlanmiyorsa loop calissin.
                rnd1 = rnd.nextInt(max + 1 - min) + min;// loto icin uretilen sayiyi buluyoruz bu sekilde.

                for (int j = 0; j < i; j++) {
                    if (lotoArr[j] == rnd1) {// rondom'u sistemin loto arrayine yerlestirmis olduk.
                        isRepeat = true;//tekrarlama yapiyorsa array'e ekleme birak. tekrarlanmiyorsa array'e ekleyecek.
                        break;
            ...
            } while (isRepeat);//tekrarlanma sarti, tekrarlanan bir sayi uretirse tekrar do'nun basina donsun. cunku tekrarli istemiyorum.
